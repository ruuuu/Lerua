...
# надоо чтобы на соыбтие был кто-то запсиан
class Zapis_on_event(unittest.TestCase):

    def registration(self, driver):

        driver.get('https://admin:LY1R1VMXJ1@leroyfactory.ru/')  # чтобы обойти базовую аунтентификацию  # страницйа авторизации
        ...
        password_d = WebDriverWait(driver, 20).until(
    ...
    def poisk_and_filter(self, driver):
        ...
        driver.execute_script("arguments[0].scrollIntoView(true);", search)  # скриллим к этому элемементу search
        time.sleep(2)
        search.click()
